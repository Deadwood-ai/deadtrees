...
class TestODMDatabaseSchema:
	"""Test ODM-related database schema changes"""

	def test_v2_raw_images_table_exists(self, auth_token):
	...
	def test_v2_raw_images_table_constraints(self, auth_token, test_user):
	...
	def test_v2_raw_images_with_rtk_data(self, auth_token, test_user):
	...
	def test_v2_raw_images_foreign_key_constraint(self, auth_token):
	...
	def test_v2_raw_images_cascade_delete(self, auth_token, test_user):
		...
		try:
			with use_client(auth_token) as client:
				...
				raw_images_check_after = (
					client.table('v2_raw_images').select('*').eq('dataset_id', dataset_id).execute()
				)
...
class TestODMEnumExtensions:
	"""Test ODM enum extensions in database"""

	def test_odm_processing_in_status_enum(self, auth_token, test_user):
	...
	def test_is_odm_done_field_in_statuses(self, auth_token, test_user):
	...
	def test_all_status_completion_flags_exist(self, auth_token, test_user):
