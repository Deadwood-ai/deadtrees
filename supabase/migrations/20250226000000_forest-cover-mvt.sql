...
CREATE OR REPLACE FUNCTION public.get_forest_cover_vector_tiles(z integer, x integer, y integer, resolution integer DEFAULT 4096)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_forest_cover_vector_tiles_debug(z integer, x integer, y integer, resolution integer DEFAULT 4096)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_forest_cover_vector_tiles_debug_extended(z integer, x integer, y integer, resolution integer DEFAULT 4096)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    IF data_srid IS NULL THEN
      RAISE NOTICE 'No SRID found in table v2_forest_cover_geometries';
