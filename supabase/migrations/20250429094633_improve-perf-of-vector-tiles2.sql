...
BEGIN
    ...
    -- Areas are in square units of the source geometry's coordinate system
    min_area := CASE
        WHEN z < 8 THEN 10000  -- Filter out very small geometries
        WHEN z < 10 THEN 1000  -- Include more geometries as we zoom in
        WHEN z < 12 THEN 100   -- Include even more at mid-zoom
        WHEN z < 14 THEN 10    -- Include most geometries
    ...
    SELECT ST_AsMVT(tile_data, 'deadwood', resolution)
      ...
      FROM (
            ...
            -- Filter by geometry area instead of using a row limit
