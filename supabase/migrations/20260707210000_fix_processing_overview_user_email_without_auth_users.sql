-- Fix the 403 on v2_processing_overview.
--
-- The view is security_invoker=true (so RLS on the base tables governs which
-- rows a caller sees), but it joined `auth.users` directly to expose
-- `user_email`. Under security_invoker the join runs with the *caller's*
-- privileges, and the `authenticated` role cannot read `auth.users`, so every
-- request failed with `42501: permission denied for table users` — the entire
-- Processing tab was broken for all authenticated users.
--
-- Source the email through the existing `get_user_emails(uuid[])` SECURITY
-- DEFINER helper instead. It reads `auth.users` as its owner (so the caller
-- needs no direct grant) and self-gates with `can_audit()`, so non-auditors get
-- no email. security_invoker=true is restated so RLS keeps governing row
-- visibility (CREATE OR REPLACE otherwise resets view options to definer).
--
-- Only the `user_email` source changes and the `auth.users` join is dropped; the
-- output column set is byte-identical, so no dependent/grant/frontend change.
CREATE OR REPLACE VIEW public.v2_processing_overview
WITH (security_invoker = true) AS
 SELECT DISTINCT ON (d.id) d.id AS dataset_id,
    d.file_name,
