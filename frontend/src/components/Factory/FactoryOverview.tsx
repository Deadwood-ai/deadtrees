...
/**
 * Operator first screen: what needs care, what is waiting and for how long, what
 * is running, then whether the platform is improving. History stays collapsed.
 */
	...
	const workerColumns: ColumnsType<FactoryWorker> = [
		{ title: "Worker", dataIndex: "worker_id", key: "worker_id", render: (value: string | null) => <span className="font-mono text-xs">{value || "unknown"}</span> },
		{
			...
			render: (value: number | null) =>
				value ? (
					<Link to={`/factory/datasets/${value}`} state={{ factoryReturnTo: "/factory" }} className="font-mono">
						#{value}
					</Link>
				) : (
					"—"
